        ...
        <tbody>
            ...
            {props.orderItems.map((item, i) => (
                <tr key={i}>
                    ...
                    <td style={{ padding: '12px 0', borderBottom: `1px solid ${border}`, color: dark, fontSize: '14px', textAlign: 'right', fontWeight: 600 }}>L.E {(item.price * item.quantity).toLocaleString()}</td>
                </tr>
            ))}
            {/* Subtotal */}
            <tr>
                <td colSpan={2} style={{ padding: '8px 0', color: muted, fontSize: '13px' }}>Subtotal</td>
                <td style={{ padding: '8px 0', textAlign: 'right', color: muted, fontSize: '13px' }}>L.E {props.subtotal.toLocaleString()}</td>
            </tr>
            {/* Shipping */}
            <tr>
                <td colSpan={2} style={{ padding: '8px 0', color: muted, fontSize: '13px' }}>Shipping</td>
                <td style={{ padding: '8px 0', textAlign: 'right', color: muted, fontSize: '13px' }}>L.E {props.shippingCost.toLocaleString()}</td>
            </tr>
            {/* COD Fee */}
            {props.codFee > 0 && (
                <tr>
                    <td colSpan={2} style={{ padding: '8px 0', color: muted, fontSize: '13px' }}>COD Fee</td>
                    <td style={{ padding: '8px 0', textAlign: 'right', color: muted, fontSize: '13px' }}>L.E {props.codFee.toLocaleString()}</td>
                </tr>
            )}
            {/* Discount */}
            {props.discountAmount > 0 && (
                <tr>
                    ...
                    <td style={{ padding: '8px 0', textAlign: 'right', color: '#16a34a', fontSize: '13px' }}>-L.E {props.discountAmount.toLocaleString()}</td>
                </tr>
            )}
            {/* Total */}
            <tr>
                <td colSpan={2} style={{ padding: '16px 0 0', fontSize: '16px', fontWeight: 700, color: dark, borderTop: `2px solid ${dark}` }}>Total</td>
                <td style={{ padding: '16px 0 0', textAlign: 'right', fontSize: '16px', fontWeight: 700, color: dark, borderTop: `2px solid ${dark}` }}>L.E {props.totalAmount.toLocaleString()}</td>
            </tr>
        </tbody>
    </table>
);
