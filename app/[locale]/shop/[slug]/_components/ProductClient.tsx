...
import { ProductDetailsTabs } from '@/app/[locale]/shop/[slug]/_components/ProductDetailsTabs'
...
export function ProductClient({ initialProduct, relatedProducts, lowStockThreshold }: Props) {
    ...
    // Initialize selected attributes only once on mount
    useEffect(() => {
                    ...
                    <div className="max-w-[1100px] mx-auto px-6 lg:px-0 py-8">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-[420px_minmax(400px,600px)_100px] gap-4 lg:gap-10 mb-8 lg:mb-16">
                            ...
                            <div className="space-y-4 order-2 lg:order-1">
                                {/* Breadcrumb — desktop */}
                                <nav aria-label="breadcrumb" className="hidden lg:block text-xs text-neutral-500 mb-4">
                                    <ol itemScope itemType="https://schema.org/BreadcrumbList" className="flex items-center">
                                        <li itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem">
                                            <Link href={`/${locale}/shop`} itemProp="item" className="hover:text-neutral-900 transition-colors">
                                        ...
                                        {product.categories?.[0] && (
                                            <>
                                                <li itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem">
                                                    <Link href={`/${locale}/shop?category=${encodeURIComponent(product.categories[0])}`} itemProp="item" className="hover:text-neutral-900 transition-colors">
                                        ...
                                        <li itemProp="itemListElement" itemScope itemType="https://schema.org/ListItem">
                                            <span itemProp="name" className="text-neutral-900" aria-current="page">{product.name}</span>
                                ...
                                <div className="flex flex-col gap-4 w-full">
                                    {allMedia.map((src, index) => (
                                        ...
                                        >
                                            <Image
                                                src={src}
                                                alt={index === 0 ? product.name : `${product.name} - ${product.categories?.[0] || ''} - صورة ${index}`}
                                                width={1000}
                                                height={1000}
