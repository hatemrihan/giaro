...
async function exportToPDF(orders: Order[]) {
    ...
    autoTable(doc, {
        head: [columns],
        body: rows.map(r => columns.map(c => String(r[c as keyof typeof r] ?? ''))),
        startY: 26,
...
export default function OrdersPage() {
    ...
    const handleExport = async (format: 'excel' | 'csv' | 'pdf' | 'doc') => {
        ...
        try {
            ...
            toast.success(`Exported ${filtered.length} orders`, { id: toastId, duration: 2000 });
        } catch (err) {
                    ...
                    <div className="space-y-6 py-2">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-3 gap-3">
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        onChange={e => setManualForm(p => ({ ...p, paymentMethod: e.target.value as 'cashOnDelivery' | 'instaPay' }))}
                            ...
                            <div className="mt-3 p-3 bg-stone-800/40 rounded-lg border border-stone-800/60 space-y-1">
                                <div className="flex justify-between text-[12px]">
                                    <span className="text-stone-400">Subtotal</span>
                                    <span className="text-stone-300 tabular-nums">{manualSubtotal.toLocaleString()} EGP</span>
                                </div>
                                <div className="flex justify-between text-[12px]">
                                    <span className="text-stone-400">Shipping</span>
                                    <span className="text-stone-300 tabular-nums">{manualForm.shippingCost.toLocaleString()} EGP</span>
                                </div>
                                {manualForm.paymentMethod === 'cashOnDelivery' && manualForm.codFee > 0 && (
                                    <div className="flex justify-between text-[12px]">
                                        <span className="text-stone-400">COD Fee</span>
                                        <span className="text-stone-300 tabular-nums">{manualForm.codFee.toLocaleString()} EGP</span>
                                    </div>
                                )}
                                <div className="flex justify-between text-[13px] font-semibold pt-1 border-t border-stone-700/50">
                                    <span className="text-white">Total</span>
                                    <span className="text-white tabular-nums">{manualTotal.toLocaleString()} EGP</span>
                                </div>
                            </div>
                        </div>
