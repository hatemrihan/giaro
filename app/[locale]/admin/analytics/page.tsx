...
import {
    DollarSign, ShoppingCart, Package, Tag, Users, Percent,
...
function formatEGP(val: number) {
    return `${val.toLocaleString('en-EG')} EGP`;
}
...
export default function AnalyticsPage() {
    ...
    const adminLink = useCallback((path: string) => `/${locale}/admin${path}`, [locale]);
    ...
    return (
        <div className="space-y-6 -mt-2 max-w-[1100px]">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                <KPICard
                    icon={DollarSign} label="Revenue" value={formatEGP(data.totalRevenue)}
            ...
            <div className="grid grid-cols-3 lg:grid-cols-6 gap-2.5">
                ...
                <MetricPill icon={Percent} label="Offers" value={`${data.activeOffers}/${data.totalOffers}`} href={adminLink('/offers')} />
                        ...
                        {(data.lowStockProducts ?? []).map(p => (
                            ...
                            >
                                ...
                                <span className={`text-[11px] font-mono font-semibold tabular-nums ${
                                    p.stock <= 2 ? 'text-red-400' : 'text-amber-400'
                                }`}>
                                    {p.stock} left
                                </span>
                            </Link>
                        ...
                        <div className="space-y-1.5">
                            {recentOrders.map(order => (
                                <div key={order.id} className="flex items-center gap-3 py-2 px-2.5 rounded-md hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <span className="text-[12px] text-white/60 font-mono shrink-0">{formatEGP(order.amount)}</span>
                            ...
                            {topCustomers.map((c) => (
                                <div key={c.email || c.name} className="flex items-center gap-3 py-1.5">
                                    ...
                                    <div className="text-right shrink-0">
                                        <span className="text-[12px] text-white/60 font-mono block">{formatEGP(c.total)}</span>
                        ...
                        <div className="space-y-1.5">
                            {recentProducts.map(p => (
                                <div key={p.id} className="flex items-center gap-3 py-2 px-1.5 rounded-md hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <span className="text-[12px] text-white/50 font-mono shrink-0">{formatEGP(p.price)}</span>
                                    {p.featured && (
            ...
            {showInsights && (
                <div className="bg-white/[0.03] border border-white/[0.06] rounded-lg p-5">
                    ...
                    <div className="space-y-2">
                        ...
                        />
                        <InsightRow
                            type={data.activeOffers === 0 ? 'info' : 'good'}
                            text={data.activeOffers === 0 ? 'No active offers — promotions drive traffic' : `${data.activeOffers} active offers`}
                        />
                        <InsightRow
